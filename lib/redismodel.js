...
module.exports = function RedisModel(collectionName, opts) {
  ...
  var indices = []
  var indexIsUseHashMap = {
    // 'indexIndex' : isIndexUseHash
  }
  ...
  function getIndexKeyForRange(sort, subsetFilter) {
    ...
    var useHash = indexIsUseHashMap[indexIndex]
    var indexOpts = [indexField, subsetFields, useHash]
  ...
  function* _ensureIndex(indexOpts) {
    ...
    var indexField = indexOpts[0]
    var subsetFields = indexOpts[1]
    var useHash = indexOpts[2] ? 1 : 0
    var indexIndex = getIndexIndex(indexField, subsetFields)
    var fullIndexIndex = indexIndex + '/hash:' + useHash
    var isIndexAdded = yield redis.sismember(KEY_INDICES, fullIndexIndex)
    if(isIndexAdded) return
    indices.push(indexOpts)
    indexIsUseHashMap[indexIndex] = useHash
    var count = 100
    var offset = 0
    while(true) {
    ...
    yield redis.sadd(KEY_INDICES, fullIndexIndex)
  }
