...
module.exports = function RedisModel(collectionName, opts) {
  ...
  function* insert(obj) {
    ...
    // Do insert
    if(obj.id) {
      var exists = yield redis.exists(KEY_ROW_PREFIX + obj.id)
      if(exists) throw new Error('already exists ' + collectionName + ':' + obj.id)
    } else {
      obj.id = yield redis.incr(KEY_SEQ)
    }
    obj.createTime = Date.now()
    yield redis.hmset(KEY_ROW_PREFIX + obj.id, obj)
    yield updateObjectIndices(obj.id, obj)
