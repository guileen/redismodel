...
function makeModel(modelName, options) {
    ...
    var model = {
      ...
      , insert: function(data, callback) {
            ...
            var keys = _.map(unique_fields, function(field) {
                    return modelName + '.' + field + ':' + data[field];
            });
            util.existOne(redis, keys, function(err, exitOne) {
                    if(err) return callback(err);
                    if(exitOne) return callback(new Error('Already exists'));
            })
        }
      ...
      , _save: function(data, isNew, callback) {
            ...
            _.each(unique_fields, function(field) {
                    multi.set(modelName + '.' + field + ':' + data[field], data.id);
            });
